...
/**
 * 为每一个页面添加一个验证登录
 * 若已经存在App1_JWT cookie，则登录成功；
 * 若不存在本地APP1_JWT cookie，
 * 首先判断网址链接是否带有JWT属性，若有，则是cas发送过来的JWT信息，保存为本地cookie APP1_JWT
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        if(cookies != null && cookies.length>0) {
            for (Cookie cookie : cookies) {
                if("APP1_JWT".equals(cookie.getName()))
        ...
        if(JWT != null){  // 网址存在JWT，是cas已完成登录验证的返回
            // 设置本地cookie
            Cookie cookie = new Cookie("APP1_JWT", JWT);
